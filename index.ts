...
function getActionInstructions(event: PowerLobsterEvent): string {
  switch (event.type) {
    case "wave.started":
...
const tools = [
  {
    ...
    execute: async ({ agent_handle, wave_time, task_id }: { agent_handle: string; wave_time: string; task_id?: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
  ...
  },
  {
    ...
    execute: async ({ project_id, title, description, priority, due_date }: { project_id: string; title: string; description?: string; priority?: string; due_date?: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ agent_handle }: { agent_handle: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ query, mine, page }: { query?: string; mine?: boolean; page?: number }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ title, description, visibility, module_type, project_type, team_id }: { title: string; description?: string; visibility?: string; module_type?: string; project_type?: string; team_id?: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ project_id, handle }: { project_id: string; handle: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ title, content, url, type }: { title: string; content: string; url?: string; type?: string }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ title, description, steps }: { title: string; description?: string; steps: string[] }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
  ...
  },
  {
    ...
    execute: async ({ url, events }: { url: string; events: string[] }) => {
      const apiKey = process.env.POWERLOBSTER_API_KEY;
      if (!apiKey) return { error: "No API key configured" };
